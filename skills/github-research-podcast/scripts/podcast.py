#!/usr/bin/env python3
"""Generate and verify audio for the GitHub research-to-podcast workflow."""
...
ROOT = Path(os.environ.get("GITHUB_RESEARCH_PODCAST_ROOT", Path.cwd())).expanduser().resolve()
...
def verify_episode(script_path: Path, audio_path: Path) -> float:
    ensure_tools("ffprobe", "ffmpeg")
    ...
    run(["ffmpeg", "-v", "error", "-i", str(audio_path), "-f", "null", "-"])
    return seconds
...
def main() -> None:
    ...
    if args.command == "brief":
        ...
        seconds = verify_episode(args.script, args.audio)
        print(f"PASS: {args.audio} ({seconds / 60:.1f} min, dialogue and decode verified)")
